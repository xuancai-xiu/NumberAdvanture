...
public class Level1Manager : MonoBehaviour
{
    ...
    private void SetupLevelUI(int level)
    {
        if (levelLabelText != null)
            levelLabelText.text = "关卡: " + level;

        if (formulaTitleText != null)
            formulaTitleText.text = "水果数值和算式";
    ...
    public void TryCompleteLevel()
    {
        ...
        {
            player.audioSource.PlayOneShot(player.reachEndSound);
            LevelComplete();
        }
        else
        {
            if (errorMessageText != null)
                errorMessageText.text = "你还没收集完水果呢！";
            player.audioSource.PlayOneShot(player.errorSound);
    ...
    {
        string nextLevelName = "Level" + (levelIndex + 1);
